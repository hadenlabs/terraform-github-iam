...
resource "github_team_membership" "this" {
  ...
  team_id    = element(github_team.teams, index(github_team.teams.*.name, element(var.members, count.index).name)).id
...
resource "github_team_repository" "this" {
  ...
  team_id    = element(github_team.teams, index(github_team.teams.*.name, element(var.permissions, count.index).team)).id
